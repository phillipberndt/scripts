...
def error_message(msg): #{{{
    ...
    """
    dialog = Gtk.MessageDialog(None, Gtk.DialogFlags.MODAL, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, msg)
...
def ask(title, question, pre_fill = False):#{{{
    ...
    my_window.set_position(Gtk.WindowPosition.MOUSE)
    my_vbox = Gtk.VBox()
    my_window.add(my_vbox)
    my_hbox = Gtk.HBox()
    my_vbox.add(my_hbox)
    my_image = Gtk.Image.new_from_icon_name("gtk-dialog-question", 3)
    my_hbox.add(my_image)
    my_label = Gtk.Label(label=question)
    ...
    my_button.set_can_default(True)
    my_button.set_image(Gtk.Image.new_from_icon_name("gtk-ok", -1))
    my_window.set_default(my_button)
    my_hbox2 = Gtk.HBox()
    my_hbox2.add(Gtk.Label())
    my_hbox2.add(my_button)
    my_hbox2.add(Gtk.Label())
...
def send_string(window, text):#{{{
    ...
    keysym_map = dict((Xlib.XK.keysym_to_string(x), x) for x in list(latin1.__dict__.values()) if type(x) is int )
    ...
    for description, password in pw_data:
        if re.search(search, description, re.I):
            if search != description:
                question_dialog = Gtk.MessageDialog(message_type=Gtk.MessageType.QUESTION, buttons=Gtk.ButtonsType.YES_NO)
